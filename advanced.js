(async function () {
    ...
    const filterAndEnrichSongsWithConstData = (constData, initialSongList, options) => {
        ...
        for (const songData of constData) {
            ...
            if (!matchesSongSelection(initialSong.title, songData.diff, targetSongRules)) continue;
    ...
    /**
    * @returns {Promise<{delay: number, scanMode: string, frameMode: string, bestConstThreshold: number, bestConstThresholdMax: number, newConstThreshold: number, newConstThresholdMax: number, songSelectionText: string, includeNewInBest: boolean, hideScore: boolean}>}
     */
    const askForSettings = () => {
        return new Promise(resolve => {
            ...
            songSelectionLabel.style.cssText = 'display: block; font-size: 18px; font-weight: bold; color: #D0D0D0; margin-bottom: 10px;';
            songSelectionSection.appendChild(songSelectionLabel);

            const songSelectionHelp = document.createElement('p');
            songSelectionHelp.innerHTML = '1行1件で指定してください。タイトルの一部で一致します。<br>「曲名|MASTER」のように難易度も付けられます。';
            songSelectionHelp.style.cssText = 'font-size: 13px; color: #B0B0B0; margin-bottom: 10px; line-height: 1.5;';
            songSelectionSection.appendChild(songSelectionHelp);

            const songSelectionInput = document.createElement('textarea');
            songSelectionInput.placeholder = '例:\nFly far bounce\nXEVEL|MASTER';
            songSelectionInput.value = songSelectionText;
            songSelectionInput.rows = 5;
            songSelectionInput.style.cssText = `
                width: 100%; box-sizing: border-box; resize: vertical;
                min-height: 120px; padding: 12px; font-size: 15px; line-height: 1.5;
                background-color: #222; color: white; border: 1px solid #555; border-radius: 8px;
            `;
            ...
            const songSuggestionStatus = document.createElement('div');
            songSuggestionStatus.style.cssText = 'margin-top: 10px; font-size: 12px; color: #9AA0A6;';
            songSuggestionStatus.textContent = '候補を読み込み中...';
            songSelectionSection.appendChild(songSuggestionStatus);

            const songSuggestionList = document.createElement('div');
            songSuggestionList.style.cssText = 'display: grid; grid-template-columns: 1fr; gap: 8px; margin-top: 10px;';
            songSelectionSection.appendChild(songSuggestionList);

            let songSuggestionEntries = [];

            const getActiveLineIndex = () => {
                const beforeCursor = songSelectionInput.value.slice(0, songSelectionInput.selectionStart || 0);
                return beforeCursor.split(/\r?\n/).length - 1;
            };

            const renderSongSuggestions = () => {
                const lines = songSelectionInput.value.split(/\r?\n/);
                const currentLineIndex = Math.max(0, Math.min(getActiveLineIndex(), lines.length - 1));
                const currentLine = lines[currentLineIndex] || '';
                const currentText = currentLine.trim();
                const [titlePartRaw, diffPartRaw = ''] = currentText.split('|');
                const titleQuery = normalizeTitle(titlePartRaw || '');
                const diffQuery = normalizeDiffSelection(diffPartRaw || '');

                songSuggestionList.innerHTML = '';
                if (!songSuggestionEntries.length) {
                    songSuggestionList.textContent = '候補はまだ読み込まれていません。';
                    return;
                }

                const filteredSuggestions = songSuggestionEntries
                    .filter(entry => (!titleQuery || entry.searchTitle.includes(titleQuery)) && (!diffPartRaw || entry.diff === diffQuery))
                    .slice(0, 8);

                if (filteredSuggestions.length === 0) {
                    songSuggestionList.textContent = '候補が見つかりません。';
                    return;
                }

                for (const entry of filteredSuggestions) {
                    const button = document.createElement('button');
                    button.type = 'button';
                    ...
                    `;
                    button.onmouseover = () => { button.style.background = 'rgba(74, 144, 226, 0.25)'; };
                    button.onmouseout = () => { button.style.background = 'rgba(255, 255, 255, 0.06)'; };
                    button.onclick = () => {
                        lines[currentLineIndex] = entry.label;
                        songSelectionInput.value = lines.join('\n');
                        songSelectionText = songSelectionInput.value;
                        renderSongSuggestions();
                    };
                    songSuggestionList.appendChild(button);
                }
            };
            ...
            (async () => {
                try {
                    const response = await fetch(CONST_DATA_URL);
                    if (!response.ok) throw new Error(`HTTP ${response.status}`);
                    const constData = await response.json();
                    songSuggestionEntries = buildSongSuggestionEntries(constData);
                    songSuggestionStatus.textContent = `候補 ${songSuggestionEntries.length} 件`;
                    renderSongSuggestions();
                } catch (e) {
                    songSuggestionStatus.textContent = '候補の読み込みに失敗しました。';
                    console.warn('Song suggestion load error:', e);
                }
            })();

            container.appendChild(songSelectionSection);
    ...
    const DIFF_SELECTION_ALIASES = {
        BASIC: 'BAS', BAS: 'BAS', '0': 'BAS',
        ADVANCED: 'ADV', ADV: 'ADV', '1': 'ADV',
        EXPERT: 'EXP', EXP: 'EXP', '2': 'EXP',
        MASTER: 'MAS', MAS: 'MAS', '3': 'MAS',
        ULTIMA: 'ULT', ULT: 'ULT', '4': 'ULT'
    };

    const normalizeDiffSelection = (value = '') => {
        const normalized = String(value || '').trim().toUpperCase();
        return DIFF_SELECTION_ALIASES[normalized] || normalized;
    };

    const parseSongSelectionText = (text = '') => {
        return String(text)
            .split(/\r?\n/)
            .map(line => line.trim())
            .filter(Boolean)
            .map(line => {
                const [titlePart, diffPart] = line.split('|').map(part => part.trim());
                return {
                    title: normalizeTitle(titlePart),
                    diff: diffPart ? normalizeDiffSelection(diffPart) : ''
                };
            })
            .filter(rule => rule.title);
    };

    const matchesSongSelection = (songTitle, songDiff, rules) => {
        if (!rules || rules.length === 0) return true;
        const normalizedTitle = normalizeTitle(songTitle);
        const normalizedDiff = String(songDiff || '').trim().toUpperCase();
        return rules.some(rule => {
            const titleMatch = normalizedTitle.includes(rule.title);
            const diffMatch = !rule.diff || rule.diff === normalizedDiff;
            return titleMatch && diffMatch;
        });
    };

    const buildSongSuggestionEntries = (constData) => {
        const seen = new Set();
        const entries = [];

        for (const item of constData || []) {
            if (!item || !item.title || !DIFF_NAME_MAP[item.diff]) continue;
            const key = `${normalizeTitle(item.title)}|${item.diff}`;
            if (seen.has(key)) continue;
            seen.add(key);
            entries.push({
                title: item.title,
                diff: item.diff,
                label: `${item.title} | ${DIFF_NAME_MAP[item.diff]}`,
                searchTitle: normalizeTitle(item.title)
            });
        }
    ...
    const fetchAllSongsForPaidUserViaRecord = async (delay, constData, options = {}) => {
        ...
        const { filteredNewSongs, filteredOldSongs } = filterAndEnrichSongsWithConstData(constData, initialSongList, {
            ...
            applyConstThreshold,
            currentVersionName,
            targetSongRules: parseSongSelectionText(songSelectionText)
        });
    ...
    // --- メイン処理 ---
    try {
        const { delay, scanMode, frameMode, bestConstThreshold, bestConstThresholdMax, newConstThreshold, newConstThresholdMax, songSelectionText, includeNewInBest, hideScore } = await askForSettings();
